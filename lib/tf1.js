 ...
 * Les credentials sont passés via /configure (pas de .env)
 * Chaque utilisateur a son propre cache et token (pas de partage)
 *
...
/** Protection globale (par IP/serveur) contre le verrouillage TF1 */
let globalAuthFailedUntil = 0;
let globalAuthFailCount = 0;
const GLOBAL_MAX_FAILS = 3;        // Max 3 échecs globaux
const GLOBAL_COOLDOWN = 60 * 60 * 1000; // 1 heure de cooldown global
...
class TF1Client {
    ...
    constructor(email = null, password = null) {
        ...
        // Credentials uniquement depuis /configure (pas de fallback .env)
        this.email = email || null;
        this.password = password || null;

        // Cache et token par instance (pas global)
        this.cache = new Map();
        this.authToken = null;
        this.authTokenExpiry = 0;

        // Protection contre les tentatives répétées (évite le verrouillage)
        this.authFailedUntil = 0;
        this.authFailCount = 0;  // Compteur d'échecs consécutifs
    }
    ...
    async _cached(key, fn) {
        ...
        this.cache.set(key, { value, expiry: now + CACHE_TTL });
        return value;
    }
     ...
     * Gère le refresh automatique et la protection contre le verrouillage
     *
    ...
    async ensureToken() {
        ...
        if (this.authToken && now < this.authTokenExpiry - 300000) {
            return this.authToken;
        }

        // Protection GLOBALE: bloquer tout le serveur après 3 échecs (évite ban IP)
        if (globalAuthFailCount >= GLOBAL_MAX_FAILS) {
            throw new Error('Auth TF1 bloquée sur ce serveur - trop d\'échecs');
        }

        // Protection GLOBALE: cooldown 1h après échec
        if (now < globalAuthFailedUntil) {
            const waitMin = Math.ceil((globalAuthFailedUntil - now) / 60000);
            throw new Error(`Auth TF1 en cooldown serveur (${waitMin} min)`);
        }

        // Protection par compte: bloquer après 2 échecs consécutifs
        if (this.authFailCount >= 2) {
            throw new Error('Auth bloquée après 2 échecs - vérifiez vos identifiants TF1');
        }

        // Protection par compte: ne pas réessayer si auth a échoué récemment (30 min)
        if (now < this.authFailedUntil) {
            const waitMin = Math.ceil((this.authFailedUntil - now) / 60000);
            throw new Error(`Auth en cooldown (${waitMin} min restantes)`);
        }
        ...
        try {
            ...
            this.authTokenExpiry = now + (tokenData.ttl * 1000);

            // Reset compteurs en cas de succès
            this.authFailedUntil = 0;
            this.authFailCount = 0;
            globalAuthFailedUntil = 0;
            globalAuthFailCount = 0;

            return this.authToken;
        } catch (error) {
            // Incrémente compteurs (par compte ET global)
            this.authFailCount++;
            globalAuthFailCount++;

            // Cooldown par compte: 30 min
            this.authFailedUntil = now + (30 * 60 * 1000);
            // Cooldown global: 1h
            globalAuthFailedUntil = now + GLOBAL_COOLDOWN;

            console.error(`[TF1] Auth échouée - compte: ${this.authFailCount}/2, serveur: ${globalAuthFailCount}/${GLOBAL_MAX_FAILS}`);
            throw error;
        }
    }
     ...
     * Note: Ne fait plus d'appels getMediaInfo pour chaque chaîne (trop lent)
     * La validation se fait au moment du stream
     *
     * @returns {Promise<Array>} Liste des chaînes en direct
     */
    async getLiveChannels() {
        return this._cached('live_channels', async () => {
            ...
            // D'abord obtenir le token (une seule fois)
            await this.ensureToken();

            // Pas d'image par défaut pour les chaînes FAST
            const defaultLogo = null;
            const lives = [];

            // Récupère les previews uniquement pour les chaînes principales (8 appels en parallèle)
            const mainChannels = allChannels.filter(c => !c.isFast);
            const mainResults = await Promise.all(
                mainChannels.map(async (channel) => {
                    try {
                        ...
                        if (info && !info.error) {
                            return {
                                ...
                                image: info.preview || channel.logo || defaultLogo,
                                logo: channel.logo || defaultLogo,
            ...
            lives.push(...mainResults.filter(r => r !== null));

            // Pour les chaînes FAST, récupère aussi les previews via getMediaInfo
            const fastOnly = allChannels.filter(c => c.isFast);
            const fastResults = await Promise.all(
                fastOnly.map(async (channel) => {
                    try {
                        const info = await this.getMediaInfo(channel.id);
                        if (info && !info.error) {
                            return {
                                ...
                                title: `${channel.name} - Direct`,
                                description: info.shortTitle || info.title || `En direct sur ${channel.name}`,
                                image: info.preview || defaultLogo,
                                logo: defaultLogo,
                                channel: channel.name,
                                isLive: true,
                                isFast: true
                    ...
                    return {
                        id: channel.id,
                        title: `${channel.name} - Direct`,
                        description: `En direct sur ${channel.name}`,
                        image: defaultLogo,
                        logo: defaultLogo,
                        channel: channel.name,
                        isLive: true,
    ...
    async getProgramsByChannel(channel) {
        return this._cached(`programs_${channel}`, async () => {
    ...
    async getVideosByProgram(programSlug, type = 'REPLAY') {
        return this._cached(`videos_${programSlug}_${type}`, async () => {
            console.log(`[TF1] Récupération vidéos ${programSlug}...`);

            // Format exact comme le plugin Kodi - limit augmenté pour avoir tous les épisodes
            const variablesStr = `{"programSlug":"${programSlug}","offset":0,"limit":500,"sort":{"type":"DATE","order":"DESC"},"types":["${type}"]}`;
            ...
            console.log(`[TF1] ${data.programBySlug.videos.items?.length || 0} vidéos pour ${programSlug}`);
    ...
    async getPopularPrograms() {
        return this._cached('popular_programs', async () => {
