...
class ArteClient {
    ...
    /**
     * Récupère les vidéos d'une zone spécifique (sous-catégorie)
     *
     * @param {string} zoneId - ID de la zone Arte
     * @param {string} pageId - ID de la page parente (ex: DOR pour documentaires)
     * @returns {Promise<Array>} Liste des vidéos
     */
    async getZone(zoneId, pageId = 'DOR') {
        return cached(`zone_${zoneId}`, async () => {
            console.log(`[Arte] Récupération zone ${zoneId}...`);
            const videos = [];

            try {
                // Récupère la première page
                const url = `${EMAC_BASE_URL}/${this.lang}/web/zones/${zoneId}/content?page=1&pageId=${pageId}&authorizedCountry=${AUTHORIZED_COUNTRY}`;
                ...
                // Récupère les pages suivantes si disponibles
                const totalPages = data.value?.pagination?.pages || 1;
                for (let page = 2; page <= Math.min(totalPages, 3); page++) {
                    try {
                        const pageUrl = `${EMAC_BASE_URL}/${this.lang}/web/zones/${zoneId}/content?page=${page}&pageId=${pageId}&authorizedCountry=${AUTHORIZED_COUNTRY}`;
                        const pageData = await this._fetch(pageUrl);
                        if (pageData.value && pageData.value.data) {
                            for (const item of pageData.value.data) {
                                if (item.programId && item.mainImage) {
                                    const hasStreams = item.availability?.hasVideoStreams !== false;
                                    if (hasStreams) {
                                        videos.push(this._formatVideo(item));
                ...
                console.log(`[Arte] ${videos.length} vidéos trouvées dans la zone`);
                return videos;
            } catch (error) {
