...
async function cached(key, fn) {
    ...
    // Ne pas cacher les résultats vides (probablement des erreurs)
    if (Array.isArray(value) && value.length === 0) {
        console.log(`[Arte] Skip cache (empty): ${key}`);
        return value;
    }
...
class ArteClient {
    ...
    async _fetch(url, options = {}) {
        try {
            ...
            // Ne pas logger si mode silencieux (ex: pagination)
            if (!options.silent) {
    ...
    /**
     * Récupère les vidéos d'une catégorie avec pagination à la demande
     *
     * @param {string} category - Code de la catégorie (ex: CIN, SER, DOR)
     * @param {number} page - Numéro de page (1-based)
     * @param {string} filterType - 'series' pour collections (RC-*), 'films' pour vidéos simples, null pour tous
     * @returns {Promise<Array>} Liste des vidéos
     */
    async getCategory(category, page = 1, filterType = null) {
        return cached(`category_${category}_p${page}_${filterType || 'all'}`, async () => {
            console.log(`[Arte] Récupération catégorie ${category} page ${page}...`);

            const videos = [];

            if (page === 1) {
                // Première page : récupère depuis la page catégorie
                const data = await this._fetch(
                    `${EMAC_BASE_URL}/${this.lang}/web/pages/${category}/?authorizedCountry=${AUTHORIZED_COUNTRY}`
                );

                if (data.value && data.value.zones) {
                    // Trouve la zone principale avec pagination
                    for (const zone of data.value.zones) {
                        if (zone.content && zone.content.data) {
                            for (const item of zone.content.data) {
                                if (item.programId && item.mainImage) {
                                    // Filtre par type si demandé
                                    const isCollection = item.programId.startsWith('RC-');
                                    if (filterType === 'series' && !isCollection) continue;
                                    if (filterType === 'films' && isCollection) continue;
                            ...
                            // Stocke le code de la zone principale pour les pages suivantes
                            if (zone.content.pagination && zone.content.pagination.pages > 1) {
                                this._categoryZones = this._categoryZones || {};
                                this._categoryZones[category] = zone.code;
                            }
                        }
                    }
                }
            } else {
                // Pages suivantes : utilise la zone principale
                const zoneCode = this._categoryZones?.[category];
                if (zoneCode) {
                    try {
                        const zoneData = await this._fetch(
                            `${EMAC_BASE_URL}/${this.lang}/web/zones/${zoneCode}/content?page=${page}&pageId=${category}&authorizedCountry=${AUTHORIZED_COUNTRY}`,
                            { silent: true } // Ne pas logger les erreurs de pagination (400 = pas de page suivante)
                        );
                        ...
                        // Erreur 400 = pagination terminée, pas une vraie erreur
            ...
            console.log(`[Arte] ${unique.length} vidéos trouvées dans ${category} page ${page}`);
            return unique;
        });
    }

    /**
     * Récupère les vidéos d'une zone spécifique (sous-catégorie) avec pagination
     *
     * @param {string} zoneId - ID de la zone Arte
     * @param {string} pageId - ID de la page parente (ex: DOR pour documentaires)
     * @param {number} page - Numéro de page (1-based)
     * @param {string} filterType - 'series' pour collections, 'films' pour vidéos simples, null pour tous
     * @returns {Promise<Array>} Liste des vidéos
     */
    async getZone(zoneId, pageId = 'DOR', page = 1, filterType = null) {
        return cached(`zone_${zoneId}_p${page}_${filterType || 'all'}`, async () => {
            console.log(`[Arte] Récupération zone ${zoneId} page ${page} (type: ${filterType || 'tous'})...`);
            const videos = [];

            try {
                const url = `${EMAC_BASE_URL}/${this.lang}/web/zones/${zoneId}/content?page=${page}&pageId=${pageId}&authorizedCountry=${AUTHORIZED_COUNTRY}`;
                ...
                console.log(`[Arte] ${videos.length} vidéos trouvées dans zone ${zoneId} page ${page}`);
                return videos;
            } catch (error) {
                console.error(`[Arte] Erreur zone ${zoneId}:`, error.message);
                return [];
            }
        });
    }
    ...
    async getCollectionEpisodes(collectionId) {
        return cached(`collection_${collectionId}`, async () => {
            ...
            try {
                ...
                const episodes = [];
                const zonesToPaginate = [];

                if (data.value && data.value.zones) {
                    for (const zone of data.value.zones) {
                        if (zone.content && zone.content.data) {
                            // Extrait le numéro de saison du titre de la zone
                            const zoneTitle = zone.title || '';
                            const seasonMatch = zoneTitle.match(/Saison\s*(\d+)/i);
                            const season = seasonMatch ? parseInt(seasonMatch[1]) : 1;

                            // Garde trace des zones avec pagination
                            const pagination = zone.content.pagination;
                            if (pagination && pagination.pages > 1) {
                                zonesToPaginate.push({
                                    code: zone.code,
                                    season,
                            ...
                            for (const item of zone.content.data) {
                                if (item.programId && !item.programId.startsWith('RC-')) {
                                    const epImgUrl = item.mainImage?.url;
                                    let epImage = null;
                                    if (epImgUrl) {
                                        epImage = epImgUrl.includes('__SIZE__')
                                            ? epImgUrl.replace('__SIZE__', '400x225')
                                            : epImgUrl;
                                    }
                                    episodes.push({
                ...
                // Récupère les pages supplémentaires pour chaque zone
                if (zonesToPaginate.length > 0) {
                    const promises = [];
                    for (const zone of zonesToPaginate) {
                        for (let page = 2; page <= Math.min(zone.totalPages, 10); page++) {
                            promises.push(
                                this._fetch(
                                    `${EMAC_BASE_URL}/${this.lang}/web/zones/${zone.code}/content?page=${page}&collectionId=${collectionId}&authorizedCountry=${AUTHORIZED_COUNTRY}`
                                ).then(pageData => ({ pageData, season: zone.season })).catch(() => null)
                            );
                        }
                    }
                    const results = await Promise.all(promises);
                    for (const result of results) {
                        if (result?.pageData?.value?.data) {
                            let episodeNum = episodes.filter(e => e.season === result.season).length + 1;
                            for (const item of result.pageData.value.data) {
                                if (item.programId && !item.programId.startsWith('RC-')) {
                                    const epImgUrl = item.mainImage?.url;
                                    let epImage = null;
                                    if (epImgUrl) {
                ...
                for (const ep of episodes) {
                    if (!seen.has(ep.programId)) {
                        seen.add(ep.programId);
                        unique.push(ep);
                    }
                }

                // Trie par saison puis épisode
                unique.sort((a, b) => (a.season - b.season) || (a.episode - b.episode));

                console.log(`[Arte] ${unique.length} épisode(s) trouvé(s) pour ${collectionId}`);
                return unique;
            } catch (error) {
     ...
     */
    _formatVideo(item) {
        const imgUrl = item.mainImage?.url;
        let poster = null;
        let background = null;

        if (imgUrl) {
            // Nettoie l'URL de base (enlève les paramètres existants)
            const baseUrl = imgUrl.split('?')[0];
            if (baseUrl.includes('__SIZE__')) {
                poster = baseUrl.replace('__SIZE__', '500x750') + '?type=TEXT';
                background = baseUrl.replace('__SIZE__', '1920x1080');
            } else {
                poster = baseUrl;
                background = baseUrl;
            }
