...
class FranceTVClient {
    ...
    /**
     * Récupère les émissions d'une chaîne (magazines, info, jeux - pas séries/films/docs)
     *
     * @param {string} channelId - ID de la chaîne
     * @returns {Promise<Array>} Liste des émissions
     */
    async getEmissions(channelId) {
        return cached(`emissions_${channelId}`, async () => {
            console.log(`[FranceTV] Récupération émissions ${channelId}...`);
            ...
            // Collections à exclure (séries, films, documentaires)
            const excludedCategories = ['séries & fictions', 'séries et fictions', 'documentaires', 'cinéma', 'films'];

            if (data.collections) {
                for (const collection of data.collections) {
                    // Vérifie si la collection est une catégorie exclue
                    const collLabel = (collection.label || '').toLowerCase();
                    const isExcluded = excludedCategories.some(cat => collLabel.includes(cat));

                    if (isExcluded) continue;

                    if (collection.items) {
                        for (const item of collection.items) {
                            // Vérifie aussi la catégorie de l'item
                            const itemCategory = (item.category?.label || '').toLowerCase();
                            const isItemExcluded = excludedCategories.some(cat => itemCategory.includes(cat));

                            if (isItemExcluded) continue;
            ...
            console.log(`[FranceTV] ${unique.length} émissions trouvées pour ${channelId}`);
            return unique;
        });
    }
    ...
    /**
     * Récupère les documentaires (tous ou par collection)
     *
     * @param {Array<number>} collectionIds - IDs des collections (optionnel)
     * @param {string} filterType - 'series' pour programmes, 'films' pour vidéos simples, null pour tous
     * @returns {Promise<Array>} Liste des documentaires
     */
    async getDocumentaries(collectionIds = null, filterType = null) {
        const cacheKey = `docs_${collectionIds ? collectionIds.join('_') : 'all'}_${filterType || 'all'}`;

        return cached(cacheKey, async () => {
            console.log(`[FranceTV] Récupération documentaires (type: ${filterType || 'tous'})...`);

            const data = await this._fetch(
                `${API_MOBILE_URL}/apps/page/documentaires?platform=apps`
            );

            const videos = [];

            // Collections à exclure (contiennent des séries fiction, pas des docs)
            const EXCLUDED_COLLECTIONS = [18867843]; // "Sélection" - contient Un si grand soleil, etc.

            if (data.collections) {
                for (const collection of data.collections) {
                    const collId = Number(collection.id);

                    // Exclure les collections non-documentaires
                    if (EXCLUDED_COLLECTIONS.includes(collId)) {
                        continue;
                    }

                    // Si des IDs sont spécifiés, ne prendre que ces collections
                    if (collectionIds && !collectionIds.includes(collId)) {
                        continue;
                    }
                    ...
                    if (collection.items) {
                        for (const item of collection.items) {
                            ...
                            if (filterType === 'series' && !video?.isProgram) continue;
                            if (filterType === 'films' && video?.isProgram) continue;
                            if (video) {
                                videos.push(video);
                            }
                        }
            ...
            console.log(`[FranceTV] ${unique.length} documentaires trouvés`);
            return unique;
        });
    }
